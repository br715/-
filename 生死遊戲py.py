...
while player_hp > 0 and ai_hp > 0:
    ...
    mag = generate_mag()
    player_items += deal_items()[:2]  #每回合新增最多兩個道具
    ai_items += deal_items()[:2]
    ...
    while mag and player_hp > 0 and ai_hp > 0:
        ...
        if turn == "player":
            ...
            if player_handcuff:#玩家用手銬
                ...
                print("禁錮對方一輪!")
            #雙方都有護盾
            if player_shield and ai_shield and bullet == "真":
                if target == "對方":
                    ai_hp -= damage
                else:
                    player_hp -= damage
        ...
        #Ai回合
            ...
            if "放大鏡" in ai_items and len(set(mag)) > 1:
                if random.random() < 0.7:
                    ...
                    target = "你" if next_bullet == "真" else "自己"
                          #ai                        #玩家
            #普攻判斷
            if target is None:
                true_count = mag.count("真")
